...
#added this function that sends the reset email link to the member's email address if it exists in the database
def send_password_reset_email(member, reset_link):
    """Send a Reset password link to the member's email."""
    msg = Message(
        subject='Reset your password - The Call',
        sender=current_app.config['MAIL_DEFAULT_SENDER'],
        recipients=[member.email]
    )
    msg.body = f'''
